package xyz.refinedev.api.tablist.util;

import com.cryptomorin.xseries.XEntityType;
import com.google.common.collect.ArrayListMultimap;
import com.google.common.collect.Lists;

import lombok.Getter;
import lombok.Setter;
import lombok.experimental.UtilityClass;

import org.bukkit.ChatColor;
import org.bukkit.entity.EntityType;
import org.bukkit.entity.Player;

import xyz.refinedev.api.skin.CachedSkin;
import xyz.refinedev.api.skin.SkinAPI;
import xyz.refinedev.api.tablist.TablistHandler;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

@Getter @Setter
@UtilityClass
public class Skin {

    private static final List<CachedSkin> DOT_SKINS = Lists.newArrayList();
    private static final ArrayListMultimap<EntityType, CachedSkin> MOB_SKINS = ArrayListMultimap.create();
    public static final Map<String, CachedSkin> GENERAL_SKINS = new HashMap<>();

    public static final CachedSkin DEFAULT_SKIN;

    public static final CachedSkin DISCORD_SKIN;
    public static final CachedSkin FACEBOOK_SKIN;
    public static final CachedSkin TWITTER_SKIN;
    public static final CachedSkin YOUTUBE_SKIN;
    public static final CachedSkin WEBSITE_SKIN;
    public static final CachedSkin CROWN_SKIN;
    public static final CachedSkin PING_SKIN;
    public static final CachedSkin NO_PING_SKIN;
    public static final CachedSkin OPEN_LOCK_SKIN;
    public static final CachedSkin CLOSED_LOCK_SKIN;

    static {
        // Default skin
        DEFAULT_SKIN = new CachedSkin(
                "default",
                "",
                ""
        );

        // Social skins + 1.8
        DISCORD_SKIN = new CachedSkin(
                "discord",
                "",
                ""
        );

        FACEBOOK_SKIN = new CachedSkin(
                "facebook",
                "",
                ""
        );
        WEBSITE_SKIN = new CachedSkin(
                "website",
                "",
                ""
        );
        TWITTER_SKIN = new CachedSkin(
                "twitter",
                "",
                ""
        );

        YOUTUBE_SKIN = new CachedSkin(
                "youtube",
                "",
                ""
        );

        CROWN_SKIN = new CachedSkin(
                "crown",
                "",
                ""
        );

        PING_SKIN = new CachedSkin(
                "ping",
                "",
                ""
        );

        NO_PING_SKIN = new CachedSkin(
                "no_ping",
                "",
                ""
        );

        OPEN_LOCK_SKIN = new CachedSkin(
                "open_lock",
                "",
                ""
        );

        CLOSED_LOCK_SKIN = new CachedSkin(
                "closed_lock",
                "",
                ""
        );

        DOT_SKINS.add(
                new CachedSkin(
                        "black",
                        "",
                        ""
                )
        );

        DOT_SKINS.add(
                new CachedSkin(
                        "dark_blue",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "dark_green",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "dark_aqua",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "dark_red",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "dark_purple",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "gold",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "gray",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "dark_gray",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "blue",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "green",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "aqua",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "red",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "light_purple",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "yellow",
                        "",
                        ""
                )
        );
        DOT_SKINS.add(
                new CachedSkin(
                        "white",
                        "",
                        ""
                )
        );

        MOB_SKINS.put(XEntityType.PIG.get(),
                new CachedSkin(
                        "pig",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.COW.get(),
                new CachedSkin(
                        "cow",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.CHICKEN.get(),
                new CachedSkin(
                        "chicken",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.WOLF.get(),
                new CachedSkin(
                        "wolf",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.OCELOT.get(),
                new CachedSkin(
                        "ocelot",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.SHEEP.get(),
                new CachedSkin(
                        "sheep",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.SQUID.get(),
                new CachedSkin(
                        "squid",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.VILLAGER.get(),
                new CachedSkin(
                        "villager",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.SLIME.get(),
                new CachedSkin(
                        "slime",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.MAGMA_CUBE.get(),
                new CachedSkin(
                        "magma_cube",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.CREEPER.get(),
                new CachedSkin(
                        "creeper",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.ZOMBIE.get(),
                new CachedSkin(
                        "zombie",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.SKELETON.get(),
                new CachedSkin(
                        "skeleton",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.SKELETON.get(),
                new CachedSkin(
                        "skeleton",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.WITHER.get(),
                new CachedSkin(
                        "wither",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.ENDERMAN.get(),
                new CachedSkin(
                        "enderman",
                        "",
                        ""
                ));
        MOB_SKINS.put(XEntityType.IRON_GOLEM.get(),
                new CachedSkin(
                        "iron_golem",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.BLAZE.get(),
                new CachedSkin(
                        "blaze",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.GHAST.get(),
                new CachedSkin(
                        "ghast",
                        "",
                        ""
                )
        );
        MOB_SKINS.put(XEntityType.SNOW_GOLEM.get(),
                new CachedSkin(
                        "snowman",
                        "",
                        ""
                )
        );

        GENERAL_SKINS.put("DEFAULT_SKIN", DEFAULT_SKIN);
        GENERAL_SKINS.put("DISCORD_SKIN", DISCORD_SKIN);
        GENERAL_SKINS.put("FACEBOOK_SKIN", FACEBOOK_SKIN);
        GENERAL_SKINS.put("WEBSITE_SKIN", WEBSITE_SKIN);
        GENERAL_SKINS.put("TWITTER_SKIN", TWITTER_SKIN);
        GENERAL_SKINS.put("YOUTUBE_SKIN", YOUTUBE_SKIN);
        GENERAL_SKINS.put("CROWN_SKIN", CROWN_SKIN);
        GENERAL_SKINS.put("PING_SKIN", PING_SKIN);
        GENERAL_SKINS.put("NO_PING_SKIN", NO_PING_SKIN);
        GENERAL_SKINS.put("OPEN_LOCK_SKIN", OPEN_LOCK_SKIN);
        GENERAL_SKINS.put("CLOSED_LOCK_SKIN", CLOSED_LOCK_SKIN);
    }

    /**
     * Get a skin from an entity type.
     *
     * @param type the entity type
     * @return A skin instance
     */
    public CachedSkin getMob(EntityType type) {
        return getMob(type, 0);
    }

    /**
     * Get a skin from an entity type and alternate form number.
     *
     * @param type          the entity type
     * @param alternateForm Used if multiple versions of this mob exist (wither skeleton = 1, ender guardian = 1)
     * @return A skin instance
     */
    public CachedSkin getMob(EntityType type, int alternateForm) {
        List<CachedSkin> skins = MOB_SKINS.get(type);

        if (skins.isEmpty()) {
            return (DEFAULT_SKIN);
        }

        if (alternateForm >= skins.size()) {
            return (DEFAULT_SKIN);
        }

        return (skins.get(alternateForm));
    }

    /**
     * Get a cool skin based on a chat color (grey square with a colored dot).
     *
     * @param color the color
     * @return A skin instance
     */
    public CachedSkin getDot(ChatColor color) {
        return (DOT_SKINS.get(color.ordinal()));
    }

    /**
     * Get a skin from an online player.
     *
     * @param player the player
     * @return A skin instance
     */
    public CachedSkin getPlayer(Player player) {
        try {
            SkinAPI skinAPI = TablistHandler.getInstance().getSkinAPI();
            CachedSkin cachedSkin = skinAPI.getSkin(player.getName());
            if (cachedSkin == SkinAPI.DEFAULT) return DEFAULT_SKIN;

            return cachedSkin;

        } catch (Exception e) {
            return (DEFAULT_SKIN);
        }
    }
}
